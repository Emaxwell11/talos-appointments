              ...
              <div class="card-body box-profile pt-4 mt-5 p-0">
                ...
                <ul class="list-group list-group-unbordered pt-3">
                  <li class="list-group-item pl-3 pr-3">
                    <b><?php echo trans('total-appointments') ?></b> <a class="float-right badge badge-secondary-soft"><?php echo count_customer_info($customer->id, 1); ?></a>
                  </li>
                  <li class="list-group-item pl-3 pr-3">
                    <b><?php echo trans('total-services') ?></b> <a class="float-right badge badge-secondary-soft"><?php echo count_customer_info($customer->id, 2); ?></a>
                  </li>
                  <li class="list-group-item pl-3 pr-3">
                    <b><?php echo trans('last-appointment') ?></b> <a class="float-right badge badge-secondary-soft"><?php if (!empty($appointments)){echo my_date_show($appointments[0]->created_at);} ?></a>
                  </li>
                </ul>
              </div>
          </div>
                          ...
                          <tr>
                            ...
                            <?php $check_payment = check_appointment_payment($appointment->id) ?>
                                <?php if ($check_payment == true): ?>
                                  <p class="mb-1 text-success"><i class="fas fa-check-circle"></i> <?php echo trans('paid') ?></p>
                                <?php else: ?>
                                  <?php if ($appointment->price != 0): ?>
                                    <p class="mb-1 text-danger"><i class="far fa-clock"></i> <?php echo trans('pending') ?></p>
                                  <?php endif; ?>
                                <?php endif; ?>
                            </td>
