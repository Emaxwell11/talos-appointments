...
class Admin_model extends CI_Model {
    ...
    // function get_transactions_by_paylink_id($service_id){
    //     $this->db->select();
    //     $this->db->from('user_pay_link');
    ...
    // Get one pay link by its id
    public function get_user_pay_link($user_pay_link_id) {
        return $this->db->get_where('user_pay_link', ['id' => $user_pay_link_id])->row_array();
    }
    
    // Get one pay link by product_id, if you still open the modal from product context
    public function get_user_pay_link_by_product($product_id) {
        return $this->db->get_where('user_pay_link', ['product_id' => $product_id])->row_array();
    }
    
    // Correct: list transactions for a given user_pay_link_id
    public function get_transactions_by_user_pay_link_id($user_pay_link_id) {
        $this->db->from('transactions');
        $this->db->where('user_pay_link_id', $user_pay_link_id);
        $this->db->order_by('id', 'DESC');
        return $this->db->get()->result_array();
    }
    
    // Optional: list transactions by product_id via join to user_pay_link
    public function get_transactions_by_product_id($product_id) {
        $this->db->select('t.*');
        $this->db->from('transactions t');
        $this->db->join('user_pay_link u', 'u.id = t.user_pay_link_id', 'inner');
        $this->db->where('u.product_id', $product_id);
        $this->db->order_by('t.id', 'DESC');
        return $this->db->get()->result_array();
    }
    
    // Get single transaction for invoice or receipt
    public function get_transaction_by_id($id) {
        $this->db->select('t.*, u.product_name, u.product_description, u.price, u.payment_description, u.customer_id');
        $this->db->from('transactions t');
        $this->db->join('user_pay_link u', 'u.id = t.user_pay_link_id', 'left');
    ...
    function get_appointments($user_id, $total, $limit, $offset)
    {
        ...
        $this->db->select('a.*, s.name as service_name, s.duration_type, s.zoom_link, s.price, s.duration, f.name as staff_name, f.thumb as staff_thumb, c.name as customer_name, c.email as customer_email, c.thumb as customer_thumb, c.phone as customer_phone');
    ...
    // get assaign days
    function get_timeslot_by_day($day_id, $business_id)
    {
        $this->db->select();
        $this->db->from('working_days');
    ...
    // get time
    function check_staff_time($time, $date)
    {
        $this->db->select();
        $this->db->from('appointments');
        $this->db->where('date', $date);
    ...
    // get assaign days
    function check_break($start, $end, $day)
    {
        $this->db->select();
        $this->db->from('working_time');
    ...
    function get_top_customers()
    {
        ...
        foreach ($query as $key => $value) {
            $this->db->select('a.*');
            $this->db->from('appointments a');
            $this->db->where('a.customer_id',$value->id);
            $query2 = $this->db->get();
            $query2 = $query2->num_rows();
            $query[$key]->total = $query2;
        }
    ...
    function get_top_staffs()
    {
        ...
        $this->db->from('appointments s');
        $this->db->where('s.business_id', $this->business->uid);
        $this->db->join('customers as c', 'c.id = s.staff_id', 'LEFT');
        ...
        foreach ($query as $key => $value) {
            $this->db->select('a.*');
            $this->db->from('appointments a');
            $this->db->where('a.staff_id',$value->id);
            $query2 = $this->db->get();
            $query2 = $query2->num_rows();
            $query[$key]->total = $query2;
        }
