...
class Common_model extends CI_Model {
    ...
    function get_row($table, $where = array()) {
        ...
        if (!empty($where)) {
            ...
            if ($query->num_rows() > 0) {
                // Return the first row (since it's a 'get_row' function)
                return $query->row();
            }
        }
    ...
    function select_options($id,$option,$table)
...

    {
        ...
        $this->db->select('a.*, s.name as service_name, s.duration_type, s.zoom_link, s.price, s.duration, f.name as staff_name, c.name as customer_name, c.email as customer_email, c.phone as customer_phone');
        ...
        if (isset($_GET['daterange']) && $_GET['daterange'] != '') {
            ...
            //echo date("Y-m-d", strtotime($date[0])).'='.date("Y-m-d", strtotime($date[1]));
